...
from bot.states import AutoAnimeDownloadStates
from bot.database.movies import (
    get_all_anime_series_list, get_movie_by_id,
    create_anime_series, find_movie_by_titles
)
...
async def process_aad_add_new(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    buttons = [
        [InlineKeyboardButton(text="🎬 uakino.best", callback_data="aad_site:uakino")],
        [InlineKeyboardButton(text="🌐 uafix.net", callback_data="aad_site:uafix")],
    ]
    await callback.message.answer(
        "🎌 <b>Автозавантаження аніме-серіалу</b>\n\nЗ якого сайту завантажити?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await state.set_state(AutoAnimeDownloadStates.choosing_site)
    await callback.answer()


# ── /autoAnimeDownload ────────────────────────────────────────────────────────

@router.message(Command("autoAnimeDownload"))
...
@router.callback_query(AutoAnimeDownloadStates.choosing_site, F.data.startswith("aad_site:"))
async def process_anime_series_site_choice(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🎌 <b>Автозавантаження аніме-серіалу</b> ({site_name})\n\n"
...
@router.callback_query(AutoAnimeDownloadStates.choosing_series_type, F.data.startswith("aad_series_type:"))
async def process_series_type(callback: CallbackQuery, state: FSMContext):
    ...
    if choice == "new":
        await callback.message.edit_text(
            f"➕ <b>Новий аніме-серіал</b>\n\nНадішли URL серіалу з {site_name}:"
        )
        await state.set_state(AutoAnimeDownloadStates.waiting_for_new_series_url)
    else:
        await callback.message.edit_text(
            f"📺 <b>Існуючий аніме-серіал</b>\n\nНадішли URL сезону з {site_name} — "
            "я розпізнаю серіал автоматично:"
        )
        await state.set_state(AutoAnimeDownloadStates.waiting_for_existing_series_url)
    await callback.answer()


# ── Новий серіал: URL → парсинг ──────────────────────────────────────────────

@router.message(AutoAnimeDownloadStates.waiting_for_new_series_url, ~F.text.startswith("/"))
async def process_new_series_url(message: Message, state: FSMContext):
    ...
    buttons = [
        [InlineKeyboardButton(text="✅ Підтвердити", callback_data="aads_meta:confirm")],
        [InlineKeyboardButton(text="✏️ Заповнити вручну", callback_data="aads_meta:manual")],
    ]
    await wait_msg.edit_text(
        "\n".join(lines),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await state.set_state(AutoAnimeDownloadStates.confirming_new_series_metadata)


# ── Новий серіал: підтвердження metadata ─────────────────────────────────────

@router.callback_query(AutoAnimeDownloadStates.confirming_new_series_metadata, F.data.startswith("aads_meta:"))
async def process_new_series_meta_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if choice == "confirm":
        ...
        await state.set_state(AutoAnimeDownloadStates.waiting_for_new_series_title)
    await callback.answer()
...
async def _ask_next_missing_series_field(message, state: FSMContext, missing: list, edit: bool = False):
    ...
    state_map = {
        "parsed_title": AutoAnimeDownloadStates.waiting_for_new_series_title,
        "parsed_title_en": AutoAnimeDownloadStates.waiting_for_new_series_title_en,
        "parsed_year": AutoAnimeDownloadStates.waiting_for_new_series_year,
        "parsed_imdb": AutoAnimeDownloadStates.waiting_for_new_series_imdb,
    }
    if edit:
...
@router.message(AutoAnimeDownloadStates.waiting_for_new_series_title, ~F.text.startswith("/"))
...
@router.message(AutoAnimeDownloadStates.waiting_for_new_series_title_en, ~F.text.startswith("/"))
...
@router.message(AutoAnimeDownloadStates.waiting_for_new_series_year, ~F.text.startswith("/"))
...
@router.message(AutoAnimeDownloadStates.waiting_for_new_series_imdb, ~F.text.startswith("/"))
...
async def _proceed_after_meta(message, state: FSMContext, edit: bool = False):
    ...
    if data.get("poster_url"):
        ...
        buttons = [
            [InlineKeyboardButton(text="⬇️ Завантажити автоматично", callback_data="aads_poster:auto")],
            [InlineKeyboardButton(text="📤 Перешлю свій", callback_data="aads_poster:manual")],
        ]
        if edit:
            await message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
        else:
            await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
        await state.set_state(AutoAnimeDownloadStates.waiting_for_new_series_poster)
    else:
...
@router.callback_query(AutoAnimeDownloadStates.waiting_for_new_series_poster, F.data.startswith("aads_poster:"))
...
@router.message(AutoAnimeDownloadStates.waiting_for_new_series_poster, F.photo)
...
@router.message(AutoAnimeDownloadStates.waiting_for_new_series_poster, ~F.text.startswith("/"))
...
async def _create_series_and_ask_season(message, state: FSMContext, poster_file_id: str, user_id: int):
    ...
    await state.set_state(AutoAnimeDownloadStates.waiting_for_season)


# ── Існуючий серіал: URL → пошук ─────────────────────────────────────────────

@router.message(AutoAnimeDownloadStates.waiting_for_existing_series_url, ~F.text.startswith("/"))
async def process_existing_series_url(message: Message, state: FSMContext):
    ...
    if title or title_en:
        existing = await find_movie_by_titles(title, title_en, content_type="anime_series")
        if existing:
            series_id = str(existing["_id"])
            await state.update_data(series_id=series_id, series_title=existing["title"])
            buttons = [
                [InlineKeyboardButton(text="✅ Так, це він", callback_data=f"aads_existing:confirm:{series_id}")],
                [InlineKeyboardButton(text="📋 Показати список", callback_data="aads_existing:list")],
            ]
            await wait_msg.edit_text(
                f"🔍 Знайдено в базі:\n\n"
            ...
            await state.set_state(AutoAnimeDownloadStates.choosing_existing_series)
            return
...
@router.callback_query(AutoAnimeDownloadStates.choosing_existing_series, F.data.startswith("aads_existing:"))
...
async def _show_existing_series(message, state: FSMContext, page: int, edit: bool = False):
    ...
    for s in items:
        ...
        buttons.append([InlineKeyboardButton(
            text=f"📺 {s['title']}",
            callback_data=f"aad_pick_series:{sid}"
        )])

    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀️", callback_data=f"aad_series_page:{page-1}"))
...
@router.callback_query(AutoAnimeDownloadStates.choosing_existing_series, F.data.startswith("aad_series_page:"))
...
@router.callback_query(AutoAnimeDownloadStates.choosing_existing_series, F.data.startswith("aad_pick_series:"))
...
@router.message(AutoAnimeDownloadStates.waiting_for_season, ~F.text.startswith("/"))
async def process_season(message: Message, state: FSMContext):
    ...
    if data.get("season_url"):
        ...
        await state.set_state(AutoAnimeDownloadStates.waiting_for_url)


@router.message(AutoAnimeDownloadStates.waiting_for_url, ~F.text.startswith("/"))
...
async def _show_dubbing_picker(message, state: FSMContext, dubbings: list, edit: bool = True):
    if not dubbings:
        ...
        if edit:
            ...
            await message.answer(text)
        await state.set_state(AutoAnimeDownloadStates.choosing_dubbing)
        return

    await state.update_data(available_dubbings=dubbings)
    buttons = [
        [InlineKeyboardButton(text=d, callback_data=f"aad_dubbing:{i}")]
...
@router.callback_query(AutoAnimeDownloadStates.choosing_dubbing, F.data.startswith("aad_dubbing:"))
...
@router.message(AutoAnimeDownloadStates.choosing_dubbing, ~F.text.startswith("/"))
...
async def _confirm_dubbing(message, state: FSMContext, dubbing: str, edit: bool):
    ...
    buttons = [
        [InlineKeyboardButton(text="▶️ Починаємо!", callback_data="aad_confirm:yes")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="aad_confirm:no")],
    ]
    await message.edit_text(
        "\n".join(info_lines),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await state.set_state(AutoAnimeDownloadStates.confirming)


# ── Підтвердження і запуск ───────────────────────────────────────────────────

@router.callback_query(AutoAnimeDownloadStates.confirming, F.data.startswith("aad_confirm:"))
async def process_confirm(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await callback.message.edit_text(
        ...
        f"Щоб зупинити: /cancelAnimeDownload"
    )
...
@router.message(Command("cancelAnimeDownload"))
...
@router.callback_query(F.data.startswith("aad_resume:"))
...
@router.callback_query(F.data.startswith("aad_resume_cancel:"))
