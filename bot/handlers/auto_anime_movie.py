...
from bot.states import AutoAnimeMovieStates
from bot.database.movies import (
    create_anime_movie, get_all_anime_movie_series_names, find_movie_by_titles
)
...
@router.message(Command("autoAnimeMovie"))
async def cmd_auto_movie(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        await message.answer("⛔️ Тільки для адміністраторів.")
        return
    buttons = [
        [InlineKeyboardButton(text="🎌 uakino.best", callback_data="aam_site:uakino")],
        [InlineKeyboardButton(text="🌐 uafix.net", callback_data="aam_site:uafix")],
    ]
    await message.answer(
        "🎌 <b>Автозавантаження аніме-фільму</b>\n\nЗ якого сайту завантажити?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await state.set_state(AutoAnimeMovieStates.choosing_site)


@router.callback_query(AutoAnimeMovieStates.choosing_site, F.data.startswith("aam_site:"))
async def process_anime_movie_site_choice(callback: CallbackQuery, state: FSMContext):
    site = callback.data.split(":")[1]
    await state.update_data(site=site)
    site_name = "uakino.best" if site == "uakino" else "uafix.net"
    await callback.message.edit_text(
        f"🎌 <b>Автозавантаження аніме-фільму</b>\n\nНадішли URL аніме-фільму з {site_name}:"
    )
    await state.set_state(AutoAnimeMovieStates.waiting_for_url)
    await callback.answer()


# ── URL → парсинг ────────────────────────────────────────────────────────────

@router.message(AutoAnimeMovieStates.waiting_for_url, ~F.text.startswith("/"))
async def process_movie_url(message: Message, state: FSMContext):
    ...
    buttons = [
        [InlineKeyboardButton(text="✅ Підтвердити", callback_data="aam_meta:confirm")],
        [InlineKeyboardButton(text="✏️ Заповнити вручну", callback_data="aam_meta:manual")],
    ]
    await wait_msg.edit_text(
        "\n".join(lines),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await state.set_state(AutoAnimeMovieStates.confirming_metadata)


# ── Підтвердження метаданих ───────────────────────────────────────────────────

@router.callback_query(AutoAnimeMovieStates.confirming_metadata, F.data.startswith("aam_meta:"))
async def process_meta_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if choice == "confirm":
        ...
        await callback.message.edit_text("Введіть українську назву аніме-фільму:")
        await state.set_state(AutoAnimeMovieStates.waiting_for_title_manual)
...
async def _ask_next_missing_field(message, state: FSMContext, missing: list, edit: bool = False):
    field = missing[0]
    prompts = {
        "title": "Введіть українську назву аніме-фільму:",
        "title_en": "Введіть англійську назву аніме-фільму:",
        "year": "Введіть рік випуску (наприклад: <code>2016</code>):",
        "imdb": "Введіть IMDB рейтинг (наприклад: <code>7.5</code>):",
    }
    state_map = {
        "title": AutoAnimeMovieStates.waiting_for_title_manual,
        "title_en": AutoAnimeMovieStates.waiting_for_title_en_manual,
        "year": AutoAnimeMovieStates.waiting_for_year_manual,
        "imdb": AutoAnimeMovieStates.waiting_for_imdb_manual,
    }
    text = prompts[field]
    if edit:
...
@router.message(AutoAnimeMovieStates.waiting_for_title_manual, ~F.text.startswith("/"))
...
@router.message(AutoAnimeMovieStates.waiting_for_title_en_manual, ~F.text.startswith("/"))
...
@router.message(AutoAnimeMovieStates.waiting_for_year_manual, ~F.text.startswith("/"))
...
@router.message(AutoAnimeMovieStates.waiting_for_imdb_manual, ~F.text.startswith("/"))
...
@router.callback_query(F.data.startswith("aam_add_new:"))
...
@router.callback_query(AutoAnimeMovieStates.confirming_duplicate, F.data.startswith("aam_dup:"))
async def process_duplicate_choice(callback: CallbackQuery, state: FSMContext):
    ...
    if not dubbings:
        ...
        await state.set_state(AutoAnimeMovieStates.choosing_dubbing)
    else:
        buttons = [
            [InlineKeyboardButton(text=d, callback_data=f"aam_dub:{i}")]
...
async def _show_dubbing_picker(message, state: FSMContext, edit: bool = False):
    ...
    if existing:
        ...
        warn_text = (
            f"⚠️ <b>Аніме-фільм вже є в базі!</b>\n\n"
        ...
        buttons = [
            [InlineKeyboardButton(text="▶️ Продовжити", callback_data="aam_dup:continue")],
            [InlineKeyboardButton(text="❌ Скасувати", callback_data="aam_dup:cancel")],
        ]
        if edit:
        ...
        await state.set_state(AutoAnimeMovieStates.confirming_duplicate)
        return
...
@router.callback_query(AutoAnimeMovieStates.choosing_dubbing, F.data.startswith("aam_dub:"))
...
@router.message(AutoAnimeMovieStates.choosing_dubbing, ~F.text.startswith("/"))
...
async def _show_series_membership(message, state: FSMContext, edit: bool = False):
    buttons = [
        [InlineKeyboardButton(text="📁 Так, належить до серії", callback_data="aam_series:yes")],
        [InlineKeyboardButton(text="🎌 Ні, самостійний фільм", callback_data="aam_series:no")],
    ]
    text = "Аніме-фільм належить до серії?"
    if edit:
        await message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    else:
        await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await state.set_state(AutoAnimeMovieStates.choosing_series_membership)


@router.callback_query(AutoAnimeMovieStates.choosing_series_membership, F.data.startswith("aam_series:"))
...
async def _show_series_picker(message, state: FSMContext, page: int, edit: bool = False):
    ...
    for abs_idx, name in enumerate(items, start=start):
        buttons.append([InlineKeyboardButton(
            text=f"📁 {name}",
            callback_data=f"aam_pickser:{abs_idx}"
        )])

    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀️", callback_data=f"aam_serpage:{page-1}"))
    ...
    buttons.append([InlineKeyboardButton(
        ...
        callback_data="aam_pickser:new"
    )])

    await state.update_data(all_series_list=all_series)
    text = "📁 <b>Виберіть серію аніме-фільмів:</b>"
    if edit:
        await message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    else:
        await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await state.set_state(AutoAnimeMovieStates.choosing_existing_series)


@router.callback_query(AutoAnimeMovieStates.choosing_existing_series, F.data.startswith("aam_serpage:"))
...
@router.callback_query(AutoAnimeMovieStates.choosing_existing_series, F.data.startswith("aam_pickser:"))
async def pick_series(callback: CallbackQuery, state: FSMContext):
    ...
    if val == "new":
        ...
        await state.set_state(AutoAnimeMovieStates.waiting_for_new_series_name)
    else:
        idx = int(val)
        data = await state.get_data()
        ...
        await state.set_state(AutoAnimeMovieStates.waiting_for_part_number)
    await callback.answer()


@router.message(AutoAnimeMovieStates.waiting_for_new_series_name, ~F.text.startswith("/"))
...
@router.message(AutoAnimeMovieStates.waiting_for_part_number, ~F.text.startswith("/"))
...
async def _show_download_confirm(message, state: FSMContext, edit: bool = False):
    ...
    buttons = [
        [InlineKeyboardButton(text="▶️ Починаємо!", callback_data="aam_confirm:yes")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="aam_confirm:no")],
    ]
    if edit:
        await message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    else:
        await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await state.set_state(AutoAnimeMovieStates.confirming_download)


@router.callback_query(AutoAnimeMovieStates.confirming_download, F.data.startswith("aam_confirm:"))
async def process_download_confirm(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    asyncio.create_task(
        _download_and_create_anime_movie(
            bot=bot,
            admin_id=admin_id,
            url=data["movie_url"],
...
async def _download_and_create_anime_movie(
    bot: Bot,
    admin_id: int,
    url: str,
    dubbing: str,
    ...
    try:
        ...
        view_url = f"https://t.me/{bot_info.username}?start=am_{movie_id}"

        done_markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📢 Зробити розсилку", callback_data=f"post_quick:anime_movie:{movie_id}")],
            [InlineKeyboardButton(text="🎬 Переглянути фільм", url=view_url)],
            [InlineKeyboardButton(text="➕ Ще аніме-фільм", callback_data="aam_add_new:movie"),
             InlineKeyboardButton(text="📺 Додати аніме-серіал", callback_data="aad_add_new:series")],
        ])
        ...
        logger.error(f"Auto anime movie download failed for '{title}': {e}")
        try:
